...
import { AgentSmith } from "./AgentSmith.js";
import {
  makeSpeechBubble,
  updateBubbleText,
  makeMatrixCanvas,
...
import {
  whichRoom,
  clampJoy,
  hitWall as hitWallAt,
  inMoveZone,
  approach,
  createResState,
  updateResState,
  RES_SCALES,
  isPaused as isPausedAt
} from "./logic.js";
...
/** Short vibration on devices that support it (mobile). */
function buzz(ms) {
  if (navigator.vibrate) navigator.vibrate(ms);
}

/** Pointer lock without raw input — the promise also rejects when lock is
 * simply unavailable (e.g. document not focused), which is not an error. */
function lockPointerPlain(el) {
  try {
...
/** Request pointer lock, preferring raw (unaccelerated) mouse input. */
function lockPointer(el) {
  if (!el || !el.requestPointerLock) return;
  try {
    var p = el.requestPointerLock({ unadjustedMovement: true });
    // Browsers without raw-input support reject the promise; fall back.
    if (p && p.catch) {
      p.catch(function () {
        lockPointerPlain(el);
      });
...
export default function MatrixGame() {
  ...
  var dmgVignetteRef = useRef(null);
  ...
  // End-of-game stings — every win/caught path funnels through these flags.
  useEffect(
    function () {
      if (gs.won) {
        sfx.stopSlowMoMusic();
  ...
  // `game` is a useRef().current object, so its identity is stable across renders.
  var spawnHitParticles = useCallback(
    function (x, y, z, color, count) {
  ...
  // Pointer handlers — a touch in the left zone becomes a floating joystick
  // centered where the finger lands; everything else = look.
  var handlePointerDown = useCallback(
    function (e) {
      ...
      if (game.movePtrId === -1 && inMoveZone(e.clientX, window.innerWidth)) {
        game.movePtrId = e.pointerId;
        game.joyOriginX = e.clientX;
        game.joyOriginY = e.clientY;
        game.moveX = 0;
        game.moveZ = 0;
        if (joyBaseRef.current) {
          // Base div is 85x85 at left:30 bottom:70, so its resting center is
          // (72.5, innerHeight - 112.5); shift it under the finger.
          var bx = e.clientX - 72.5;
          var by = e.clientY - (window.innerHeight - 112.5);
          joyBaseRef.current.style.transform = "translate(" + bx + "px," + by + "px)";
        }
  ...
  var handlePointerMove = useCallback(
    function (e) {
      ...
      if (e.pointerId === game.movePtrId) {
        var cl = clampJoy(e.clientX - game.joyOriginX, e.clientY - game.joyOriginY);
        game.moveX = cl.x / JOY_RADIUS;
        game.moveZ = cl.y / JOY_RADIUS;
        if (leftKnobRef.current)
          leftKnobRef.current.style.transform = "translate(" + cl.x + "px," + cl.y + "px)";
      }
      if (e.pointerId === game.lookPtrId) {
        ...
        game.pitch -=
          ...
          (settingsRef.current.invertY ? -1 : 1);
  ...
  var handlePointerUp = useCallback(
    function (e) {
      ...
      if (e.pointerId === game.movePtrId) {
        ...
        game.moveZ = 0;
        if (leftKnobRef.current) leftKnobRef.current.style.transform = "translate(0px,0px)";
        if (joyBaseRef.current) joyBaseRef.current.style.transform = "translate(0px,0px)";
      }
      if (e.pointerId === game.lookPtrId) {
        game.lookPtrId = -1;
      }
  ...
  useEffect(
    function () {
      ...
      var camera = new THREE.PerspectiveCamera(settingsRef.current.fov, W / H, 0.1, 100);
      var playerSpawnX = 10;
      var playerSpawnZ = 3;
      ...
      var renderer = new THREE.WebGLRenderer({
        // MSAA only on low-DPI screens — high-DPI density already hides
        // aliasing, and skipping it there saves fill rate.
      ...
      function effectivePixelRatio() {
        // Cap at 2 (3x panels render 2.25x the pixels of 2x for little visible
        // gain), then apply the dynamic-resolution scale.
        var dpr = window.devicePixelRatio || 1;
        return Math.min(dpr, 2) * RES_SCALES[game.resState.level];
      }
      ...
      // Textures (2 shared)
      var textures = [];
      var texSize = game.isMobile ? 128 : 192;
      ...
      function matMat(idx, emCol) {
        return new THREE.MeshBasicMaterial({
          map: textures[idx % 2].tex,
          color: emCol || 0x66ff88,
          transparent: true,
          opacity: 0.95,
      ...
      for (var wi = 0; wi < WALL_SEGS.length; wi++) {
        ...
        if (seg.a === "x") {
          ...
          wm.position.set(seg.p, ROOM_H / 2, (seg.mn + seg.mx) / 2);
        } else {
          wm.position.set((seg.mn + seg.mx) / 2, ROOM_H / 2, seg.p);
        }
        scene.add(wm);
      }
      ...
      var hallFloor = new THREE.Mesh(
        new THREE.PlaneGeometry(4, TRINITY_HALL_LEN),
        matMat(0, 0x44ff77)
      );
      ...
      lipL.position.set(4, ROOM_H / 2, 41.98);
      scene.add(lipL);
      var lipR = new THREE.Mesh(new THREE.PlaneGeometry(2, ROOM_H), matMat(2, 0x44dd77));
      lipR.position.set(10, ROOM_H / 2, 41.98);
      scene.add(lipR);
      var hallToTrinityGlow = new THREE.Mesh(
        new THREE.PlaneGeometry(4, ROOM_H - 0.5),
      ...
      scene.add(
        new THREE.PointLight(0xff66cc, 0.95, 20)
          .translateX(7)
          .translateY(ROOM_H - 1)
          .translateZ(50)
      );
      scene.add(
        new THREE.PointLight(0x00ff41, 0.65, 16).translateX(7).translateY(2).translateZ(58)
      );
      ...
      game.finalExitGlow = new THREE.Mesh(
        new THREE.PlaneGeometry(3.45, ROOM_H - 0.85),
        new THREE.MeshBasicMaterial({
          color: 0x88ffff,
          transparent: true,
          opacity: 0,
      ...
      game.finalExitCore = new THREE.Mesh(
        new THREE.PlaneGeometry(2.4, ROOM_H * 0.48),
        new THREE.MeshBasicMaterial({
          color: 0xccffff,
          transparent: true,
          opacity: 0.42,
      ...
      game.finalExitSign = new THREE.Sprite(
        new THREE.SpriteMaterial({ map: finSignTex, transparent: true, depthTest: false })
      );
      ...
      var finFMat = new THREE.MeshBasicMaterial({
        color: 0x00ffcc,
        transparent: true,
        opacity: 0.95,
        side: THREE.DoubleSide,
      ...
      var jailMat = new THREE.MeshBasicMaterial({
        color: 0x353535,
        transparent: true,
        opacity: 0.93
      });
      ...
      jTopL2.position.set(-0.92, 2.38, 0.05);
      trinityJail.add(jTopL2);
      var jTopR2 = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.06, 1.75), jailMat);
      jTopR2.position.set(0.92, 2.38, 0.05);
      trinityJail.add(jTopR2);
      var jTopN2 = new THREE.Mesh(new THREE.BoxGeometry(1.75, 0.06, 0.06), jailMat);
      jTopN2.position.set(0, 2.38, 0.9);
      trinityJail.add(jTopN2);
      ...
      var tSkin = new THREE.MeshBasicMaterial({
        color: 0xc7a48b,
        transparent: true,
        opacity: 0.99
      });
      var tHair = new THREE.MeshBasicMaterial({
        color: 0x0f0f12,
        transparent: true,
        opacity: 0.98
      });
      var tLeather = new THREE.MeshBasicMaterial({
        color: 0x050505,
        transparent: true,
        opacity: 0.99
      });
      var tLeatherHi = new THREE.MeshBasicMaterial({
        color: 0x0b0b0b,
        transparent: true,
        opacity: 0.99
      });
      var tBoot = new THREE.MeshBasicMaterial({
        color: 0x020202,
        transparent: true,
        opacity: 0.99
      });
      ...
      tHead.position.set(0, 1.68, 0);
      trinBody.add(tHead);
      // Sunglasses (small ovals)
      var tFrame = new THREE.MeshBasicMaterial({
        color: 0x050505,
        transparent: true,
        opacity: 0.99
      });
      var tLens = new THREE.MeshBasicMaterial({
        color: 0x000000,
        transparent: true,
        opacity: 0.78
      });
      ...
      var hairCap = new THREE.Mesh(
        new THREE.SphereGeometry(0.176, 12, 10, 0, Math.PI * 2, 0, Math.PI * 0.55),
        tHair
      );
      ...
      hallDoor.position.set(7, ROOM_H / 2, 28);
      scene.add(hallDoor);
      ...
      for (var di = 0; di < DOOR_DEFS.length; di++) {
        ...
        var dm = new THREE.Mesh(
          new THREE.PlaneGeometry(dl, ROOM_H),
          new THREE.MeshBasicMaterial({
            color: dc,
            transparent: true,
            opacity: 0.5,
        ...
        if (dd.a === "x") {
          ...
          dm.position.set(dd.p, ROOM_H / 2, (dd.mn + dd.mx) / 2);
        } else {
          dm.position.set((dd.mn + dd.mx) / 2, ROOM_H / 2, dd.p);
        }
        scene.add(dm);
        var ef = new THREE.Mesh(
          new THREE.PlaneGeometry(dl, 0.15),
          new THREE.MeshBasicMaterial({
            color: dc,
            transparent: true,
            opacity: 0.6,
        ...
        ef.position.copy(dm.position);
        ef.rotation.copy(dm.rotation);
        ef.position.y = ROOM_H;
        scene.add(ef);
        var ef2 = ef.clone();
        ef2.position.y = 0.08;
        scene.add(ef2);
      ...
      for (var fi = 0; fi < FURNITURE.length; fi++) {
        ...
        fm.position.set(f.x, f.h / 2 + yOffset, f.z);
        scene.add(fm);
      ...
      var gunMetal = new THREE.MeshBasicMaterial({
        color: 0x555555,
        transparent: true,
        opacity: 0.98
      });
      var gunDark = new THREE.MeshBasicMaterial({
        color: 0x1a1a1a,
        transparent: true,
        opacity: 0.98
      });
      ...
      gcFront.position.set(0, 0.9, 0.6);
      glassCase.add(gcFront);
      var gcBack = new THREE.Mesh(new THREE.PlaneGeometry(1.2, 1.8), glassMat);
      gcBack.position.set(0, 0.9, -0.6);
      glassCase.add(gcBack);
      var gcLeft = new THREE.Mesh(new THREE.PlaneGeometry(1.2, 1.8), glassMat);
      gcLeft.rotation.y = Math.PI / 2;
      gcLeft.position.set(-0.6, 0.9, 0);
      glassCase.add(gcLeft);
      var gcRight = new THREE.Mesh(new THREE.PlaneGeometry(1.2, 1.8), glassMat);
      gcRight.rotation.y = Math.PI / 2;
      gcRight.position.set(0.6, 0.9, 0);
      glassCase.add(gcRight);
      var gcTop = new THREE.Mesh(new THREE.PlaneGeometry(1.2, 1.2), glassMat);
      gcTop.rotation.x = Math.PI / 2;
      gcTop.position.set(0, 1.8, 0);
      glassCase.add(gcTop);
      var pedestal = new THREE.Mesh(
        new THREE.BoxGeometry(1.3, 0.15, 1.3),
        new THREE.MeshBasicMaterial({ color: 0x444444, opacity: 0.9, transparent: true })
      );
      pedestal.position.set(0, 0.075, 0);
      glassCase.add(pedestal);
      var gunFloorGlow = new THREE.Mesh(
        new THREE.TorusGeometry(0.8, 0.03, 8, 24),
        new THREE.MeshBasicMaterial({ color: 0x00ccff, transparent: true, opacity: 0.3 })
      );
      gunFloorGlow.rotation.x = -Math.PI / 2;
      gunFloorGlow.position.set(0, 0.02, 0);
      glassCase.add(gunFloorGlow);
      ...
      var ehGlassMat = new THREE.MeshBasicMaterial({
        color: 0xaaeeff,
        transparent: true,
        opacity: 0.28,
        side: THREE.DoubleSide
      ...
      ehFront.position.set(0, 0.92, 0.65);
      exitHallCase.add(ehFront);
      var ehBack = new THREE.Mesh(new THREE.PlaneGeometry(1.35, 1.85), ehGlassMat);
      ehBack.position.set(0, 0.92, -0.65);
      exitHallCase.add(ehBack);
      var ehLeft = new THREE.Mesh(new THREE.PlaneGeometry(1.35, 1.85), ehGlassMat);
      ehLeft.rotation.y = Math.PI / 2;
      ehLeft.position.set(-0.65, 0.92, 0);
      exitHallCase.add(ehLeft);
      var ehRight = new THREE.Mesh(new THREE.PlaneGeometry(1.35, 1.85), ehGlassMat);
      ehRight.rotation.y = Math.PI / 2;
      ehRight.position.set(0.65, 0.92, 0);
      exitHallCase.add(ehRight);
      var ehTop = new THREE.Mesh(new THREE.PlaneGeometry(1.35, 1.35), ehGlassMat);
      ehTop.rotation.x = Math.PI / 2;
      ehTop.position.set(0, 1.85, 0);
      exitHallCase.add(ehTop);
      var ehPed = new THREE.Mesh(
        new THREE.BoxGeometry(1.4, 0.16, 1.4),
        new THREE.MeshBasicMaterial({ color: 0x3a3a3a, opacity: 0.92, transparent: true })
      );
      ehPed.position.set(0, 0.08, 0);
      exitHallCase.add(ehPed);
      var ehGlow = new THREE.Mesh(
        new THREE.TorusGeometry(0.85, 0.035, 8, 24),
        new THREE.MeshBasicMaterial({ color: 0xff6644, transparent: true, opacity: 0.35 })
      );
      ehGlow.rotation.x = -Math.PI / 2;
      ehGlow.position.set(0, 0.02, 0);
      exitHallCase.add(ehGlow);
      var mgMetal = new THREE.MeshBasicMaterial({
        color: 0x3a3a3a,
        transparent: true,
        opacity: 0.98
      });
      var mgDark = new THREE.MeshBasicMaterial({
        color: 0x1a1a1a,
        transparent: true,
        opacity: 0.98
      });
      ...
      uzBody.position.set(0, 0.02, 0.02);
      machineGun.add(uzBody);
      var uzTop = new THREE.Mesh(new THREE.BoxGeometry(0.07, 0.04, 0.18), mgDark);
      uzTop.position.set(0, 0.065, 0.03);
      machineGun.add(uzTop);
      var uzCharge = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.04, 8), mgMetal);
      uzCharge.rotation.z = Math.PI / 2;
      uzCharge.position.set(0.045, 0.08, -0.02);
      machineGun.add(uzCharge);
      var uzBarrel = new THREE.Mesh(new THREE.CylinderGeometry(0.014, 0.014, 0.14, 10), mgDark);
      uzBarrel.rotation.x = Math.PI / 2;
      uzBarrel.position.set(0, 0.03, 0.22);
      machineGun.add(uzBarrel);
      var uzMuzzle = new THREE.Mesh(new THREE.CylinderGeometry(0.017, 0.017, 0.04, 8), mgMetal);
      uzMuzzle.rotation.x = Math.PI / 2;
      uzMuzzle.position.set(0, 0.03, 0.31);
      machineGun.add(uzMuzzle);
      var uzGrip = new THREE.Mesh(new THREE.BoxGeometry(0.055, 0.11, 0.055), mgDark);
      uzGrip.position.set(0, -0.1, -0.06);
      uzGrip.rotation.x = -0.35;
      machineGun.add(uzGrip);
      var uzMag = new THREE.Mesh(new THREE.BoxGeometry(0.045, 0.14, 0.065), mgDark);
      uzMag.position.set(0, -0.12, 0.1);
      uzMag.rotation.x = 0.12;
      machineGun.add(uzMag);
      var uzStockRod = new THREE.Mesh(new THREE.CylinderGeometry(0.011, 0.011, 0.16, 8), mgMetal);
      uzStockRod.rotation.x = Math.PI / 2;
      uzStockRod.position.set(0, 0.025, -0.16);
      machineGun.add(uzStockRod);
      var uzStockPlate = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.07, 0.02), mgDark);
      uzStockPlate.position.set(0, 0.025, -0.25);
      machineGun.add(uzStockPlate);
      var uzTrig = new THREE.Mesh(
        new THREE.TorusGeometry(0.022, 0.005, 6, 12, Math.PI * 0.85),
        mgDark
      );
      uzTrig.rotation.x = Math.PI / 2;
      uzTrig.position.set(0, -0.04, -0.01);
      machineGun.add(uzTrig);
      ...
      function randomKeyPos(rx, rz) {
        ...
        for (var attempt = 0; attempt < 30; attempt++) {
          ...
          for (var fi2 = 0; fi2 < furnHits.length; fi2++) {
            ...
            if (Math.abs(kx - fh2.x) < fh2.hw + 0.5 && Math.abs(kz - fh2.z) < fh2.hd + 0.5) {
              blocked = true;
              break;
            }
          }
      ...
      for (var ki = 0; ki < KEY_DEFS.length; ki++) {
        ...
        keyGroup.add(bow);
        // Bow fill
        var bowFill = new THREE.Mesh(
          new THREE.CylinderGeometry(0.11, 0.11, 0.04, 8),
          new THREE.MeshBasicMaterial({ color: keyColor, transparent: true, opacity: 0.3 })
        );
        ...
        tooth1.position.set(0.05, -0.12, 0);
        keyGroup.add(tooth1);
        var tooth2 = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.04, 0.03), toothMat);
        tooth2.position.set(0.06, -0.2, 0);
        keyGroup.add(tooth2);
        var tooth3 = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.04, 0.03), toothMat);
        tooth3.position.set(0.04, -0.05, 0);
        keyGroup.add(tooth3);

        // Edge wireframe
        var bowEdge = new THREE.LineSegments(
          new THREE.EdgesGeometry(new THREE.TorusGeometry(0.15, 0.04, 8, 16)),
          new THREE.LineBasicMaterial({ color: keyColor, opacity: 1.0 })
        );
        bowEdge.position.y = 0.25;
        keyGroup.add(bowEdge);
        ...
        var floorGlow = new THREE.Mesh(
          new THREE.TorusGeometry(0.6, 0.03, 8, 24),
      ...
      var suitEdgeMat = new THREE.LineBasicMaterial({
        color: 0x2c2c2c,
        transparent: true,
        opacity: 0.75
      });
      ...
      for (var ai = 0; ai < AGENT_SPAWNS.length; ai++) {
        ...
        // Black hair on top
        var hairMat = new THREE.MeshBasicMaterial({
          color: 0x111111,
          transparent: true,
          opacity: 0.95
        });
        var hair = new THREE.Mesh(
          new THREE.SphereGeometry(0.19, 6, 4, 0, Math.PI * 2, 0, Math.PI * 0.45),
          hairMat
        );
        hair.position.set(0, 1.72, -0.02);
        ag.add(hair);
        // Sunglasses — black visor
        var glassesMat = new THREE.MeshBasicMaterial({
          color: 0x000000,
          transparent: true,
          opacity: 0.95
        });
        var glassesFrame = new THREE.Mesh(new THREE.BoxGeometry(0.38, 0.09, 0.1), glassesMat);
        glassesFrame.position.set(0, 1.74, 0.16);
        ag.add(glassesFrame);
        var lensL = new THREE.Mesh(
          new THREE.BoxGeometry(0.14, 0.1, 0.05),
          new THREE.MeshBasicMaterial({ color: 0x050505, transparent: true, opacity: 0.95 })
        );
        lensL.position.set(-0.09, 1.74, 0.19);
        ag.add(lensL);
        var lensR = lensL.clone();
        lensR.position.set(0.09, 1.74, 0.19);
        ag.add(lensR);
        // Nose bridge
        var bridge = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.04, 0.05), glassesMat);
        bridge.position.set(0, 1.73, 0.19);
        ag.add(bridge);
        // Ear pieces
        var earL = new THREE.Mesh(new THREE.BoxGeometry(0.025, 0.025, 0.18), glassesMat);
        earL.position.set(-0.19, 1.74, 0.1);
        ag.add(earL);
        var earR = earL.clone();
        earR.position.set(0.19, 1.74, 0.1);
        ag.add(earR);
        // Red eye glow behind glasses
        var eyeGlowL = new THREE.Mesh(new THREE.SphereGeometry(0.02, 6, 6), eyeMat);
        eyeGlowL.position.set(-0.07, 1.74, 0.17);
        ag.add(eyeGlowL);
        var eyeGlowR = new THREE.Mesh(new THREE.SphereGeometry(0.02, 6, 6), eyeMat);
        eyeGlowR.position.set(0.07, 1.74, 0.17);
        ag.add(eyeGlowR);
        // Neck
        addPart(
          ag,
          new THREE.CylinderGeometry(0.07, 0.09, 0.12, 6),
          suitMat,
          0,
          1.58,
          0,
        ...
        addPart(
          ag,
          new THREE.BoxGeometry(0.5, 0.45, 0.22),
          suitMat,
          0,
          1.3,
          0,
        ...
        addPart(
          ag,
          new THREE.BoxGeometry(0.38, 0.15, 0.2),
          suitMat,
          0,
          0.72,
          0,
        ...
        var suitDark = new THREE.MeshBasicMaterial({
          color: 0x0a0a0a,
          transparent: true,
          opacity: 0.96
        });
        var tieMat = new THREE.MeshBasicMaterial({
          color: 0x080808,
          transparent: true,
          opacity: 0.95
        });
        ...
        buckle.position.set(0, 0.78, 0.12);
        ag.add(buckle);
        // Pocket square
        var pocket = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.04, 0.02), whiteMat);
        pocket.position.set(-0.18, 1.45, 0.12);
        ag.add(pocket);
        // Jacket edges
        var jeCol = new THREE.MeshBasicMaterial({
          color: 0x222222,
          opacity: 0.6,
          transparent: true
        });
        var jeL = new THREE.Mesh(new THREE.BoxGeometry(0.01, 0.5, 0.01), jeCol);
        jeL.position.set(-0.12, 1.25, 0.12);
        ag.add(jeL);
        var jeR = jeL.clone();
        jeR.position.set(0.12, 1.25, 0.12);
        ag.add(jeR);
        ...
        addPart(
          leftArmPivot,
          new THREE.CylinderGeometry(0.05, 0.045, 0.3, 6),
          suitMat,
          -0.02,
          -0.48,
          0.03,
        ...
        addPart(
          leftLegPivot,
          new THREE.CylinderGeometry(0.08, 0.07, 0.4, 6),
          suitMat,
          0,
          -0.2,
          0,
        ...
        addPart(
          leftLegPivot,
          new THREE.CylinderGeometry(0.065, 0.06, 0.35, 6),
          suitMat,
          0,
          -0.52,
          0.02,
        ...
        game.agents.push({
          ...
          x: asp.x,
          z: asp.z,
          minX: asp.rx + AGENT_MARGIN,
          maxX: asp.rx + rw - AGENT_MARGIN,
          minZ: asp.rz + AGENT_MARGIN,
          maxZ: asp.rz + rd - AGENT_MARGIN,
          roomId: asp.room,
          speed: asp.speed,
          targetX: patrolX,
          targetZ: patrolZ,
      ...
      game.cats = [];
      var catMat = new THREE.MeshBasicMaterial({
        color: 0x222222,
        transparent: true,
        opacity: 0.9
      });
      ...
      for (ci = 0; ci < catSpawns.length; ci++) {
        var cs = catSpawns[ci];
        ...
        var catLeg1 = new THREE.Mesh(legGeo, catMat);
        catLeg1.position.set(-0.07, 0.08, 0.12);
        catGrp.add(catLeg1);
        var catLeg2 = new THREE.Mesh(legGeo, catMat);
        catLeg2.position.set(0.07, 0.08, 0.12);
        catGrp.add(catLeg2);
        var catLeg3 = new THREE.Mesh(legGeo, catMat);
        catLeg3.position.set(-0.07, 0.08, -0.12);
        catGrp.add(catLeg3);
        var catLeg4 = new THREE.Mesh(legGeo, catMat);
        catLeg4.position.set(0.07, 0.08, -0.12);
        catGrp.add(catLeg4);
        ...
        game.cats.push({
          ...
          minZ: 0.8,
          maxZ: 13.2,
          targetX: 1 + Math.random() * 12,
          targetZ: 1 + Math.random() * 12,
          patrolTimer: 0,
          bubble: catBubble,
      ...
      ectx.strokeStyle = "#00ff41";
      ectx.lineWidth = 2;
      ectx.strokeRect(2, 2, 124, 44);
      ectx.font = "bold 28px monospace";
      ectx.fillStyle = "#00ff41";
      ectx.textAlign = "center";
      ectx.fillText("EXIT", 64, 35);
      var exitTex = new THREE.CanvasTexture(exitCanvas);
      var exitSign = new THREE.Sprite(
        new THREE.SpriteMaterial({ map: exitTex, transparent: true })
      );
      exitSign.scale.set(1.2, 0.45, 1);
      exitSign.position.set(7, ROOM_H - 0.5, 27.5);
      scene.add(exitSign);
      scene.add(
        new THREE.PointLight(0x00ff41, 0.5, 4)
          .translateX(7)
          .translateY(ROOM_H - 0.8)
          .translateZ(27.5)
      );
      ...
      var phoneHandset = new THREE.Mesh(
        new THREE.BoxGeometry(0.06, 0.04, 0.18),
        new THREE.MeshBasicMaterial({ color: 0x111111 })
      );
      phoneHandset.position.y = 0.04;
      phoneGrp.add(phoneHandset);
      var phoneLight = new THREE.PointLight(0x00ff41, 0.5, 3);
      phoneGrp.add(phoneLight);
      scene.add(phoneGrp);
      ...
      morpheusRoot.rotation.y = Math.atan2(
        playerSpawnX - morpheusRoot.position.x,
        playerSpawnZ - morpheusRoot.position.z
      );
      ...
      morHead.position.set(0, 1.74, 0);
      morpheusRoot.add(morHead);
      var morLensL = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.056, 0.042), morShades);
      morLensL.position.set(-0.085, 1.752, 0.152);
      morpheusRoot.add(morLensL);
      var morLensR = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.056, 0.042), morShades);
      morLensR.position.set(0.085, 1.752, 0.152);
      morpheusRoot.add(morLensR);
      var morSpecBr = new THREE.Mesh(new THREE.BoxGeometry(0.048, 0.022, 0.028), morShades);
      morSpecBr.position.set(0, 1.752, 0.138);
      morpheusRoot.add(morSpecBr);
      var morTempleL = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.018, 0.12), morShades);
      morTempleL.position.set(-0.195, 1.752, 0.1);
      morTempleL.rotation.z = 0.12;
      morpheusRoot.add(morTempleL);
      var morTempleR = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.018, 0.12), morShades);
      morTempleR.position.set(0.195, 1.752, 0.1);
      morTempleR.rotation.z = -0.12;
      morpheusRoot.add(morTempleR);
      var morTorso = new THREE.Mesh(new THREE.CylinderGeometry(0.19, 0.22, 0.5, 8), morLeather);
      morTorso.position.set(0, 1.36, 0);
      morpheusRoot.add(morTorso);
      var morShoulders = new THREE.Mesh(new THREE.BoxGeometry(0.52, 0.14, 0.26), morLeather);
      morShoulders.position.set(0, 1.58, 0.02);
      morpheusRoot.add(morShoulders);
      var morCollar = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.1, 0.2), morLeatherHi);
      morCollar.position.set(0, 1.82, -0.06);
      morCollar.rotation.x = 0.35;
      morpheusRoot.add(morCollar);
      var morLapelL = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.36, 0.04), morLeatherHi);
      morLapelL.position.set(-0.1, 1.42, 0.17);
      morLapelL.rotation.z = 0.22;
      morLapelL.rotation.y = -0.08;
      morpheusRoot.add(morLapelL);
      var morLapelR = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.36, 0.04), morLeatherHi);
      morLapelR.position.set(0.1, 1.42, 0.17);
      morLapelR.rotation.z = -0.22;
      morLapelR.rotation.y = 0.08;
      morpheusRoot.add(morLapelR);
      var morHips = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.2, 0.48, 8), morLeather);
      morHips.position.set(0, 0.96, 0.04);
      morpheusRoot.add(morHips);
      var morSkirtL = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.42, 0.06), morLeather);
      morSkirtL.position.set(-0.16, 0.62, 0.08);
      morSkirtL.rotation.z = 0.08;
      morSkirtL.rotation.x = -0.12;
      morpheusRoot.add(morSkirtL);
      var morSkirtR = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.42, 0.06), morLeather);
      morSkirtR.position.set(0.16, 0.62, 0.08);
      morSkirtR.rotation.z = -0.08;
      morSkirtR.rotation.x = -0.12;
      morpheusRoot.add(morSkirtR);
      var morArmL = new THREE.Mesh(new THREE.CylinderGeometry(0.068, 0.058, 0.58, 6), morLeather);
      morArmL.position.set(-0.27, 1.2, 0);
      morpheusRoot.add(morArmL);
      var morArmR = new THREE.Mesh(new THREE.CylinderGeometry(0.068, 0.058, 0.58, 6), morLeather);
      morArmR.position.set(0.27, 1.2, 0);
      morpheusRoot.add(morArmR);
      ...
      morLegL.position.set(-0.11, 0.39, 0);
      morpheusRoot.add(morLegL);
      var morLegR = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.065, 0.78, 6), morBoot);
      morLegR.position.set(0.11, 0.39, 0);
      morpheusRoot.add(morLegR);
      ...
      var boothMat = new THREE.MeshBasicMaterial({
        color: 0xcc2222,
        transparent: true,
        opacity: 0.7
      });
      var boothFrame = new THREE.MeshBasicMaterial({
        color: 0x555555,
        transparent: true,
        opacity: 0.8
      });
      var boothGlass = new THREE.MeshBasicMaterial({
        color: 0x224444,
        transparent: true,
        opacity: 0.3,
        side: THREE.DoubleSide
      ...
      telCtx.fillStyle = "rgba(0,0,0,0.9)";
      telCtx.fillRect(0, 0, 256, 48);
      telCtx.font = "bold 16px monospace";
      telCtx.fillStyle = "#00ff41";
      ...
      telSign.scale.set(1.0, 0.3, 1);
      telSign.position.set(0, 3.0, 0);
      ...
      function makeProxLabel(text, x, y, z, color) {
        ...
        var lsprite = new THREE.Sprite(
          new THREE.SpriteMaterial({ map: ltex, transparent: true, depthTest: false })
        );
      ...
      game.proxLabels = [
        {
          sprite: makeProxLabel('"There is no spoon"', -8, 2.2, 5, "#cccccc"),
          x: -8,
          z: 5,
          range: 4
        },
        ...
        {
          sprite: makeProxLabel("Follow the white rabbit", 20, 0.7, 7, "#ffffff"),
          x: 20,
          z: 7,
          range: 5
        },
        {
          sprite: makeProxLabel("Ring ring... pick up!", 10, 1.5, -10, "#00ff41"),
          x: 10,
          z: -10,
          range: 4
        },
        {
          sprite: makeProxLabel("The line is open, Neo...", 1.5, 2.0, 11.5, "#00ff41"),
          x: 1.5,
          z: 11.8,
          range: 2.5
        },
        {
          sprite: makeProxLabel("Break glass — collect all keys (pistol)", 7, 2.0, 10.5, "#00ccff"),
          x: 7,
          z: 10.5,
          range: 3
        },
        {
          sprite: makeProxLabel(
            "Armory — clear Exit Hall, then grab the MG",
            EXIT_HALL_CASE_X,
            2.1,
            EXIT_HALL_CASE_Z,
            "#ffaa66"
      ...
      function hitWall(nx, nz) {
        return hitWallAt(nx, nz, {
          opened: game.opened,
          furnHits: furnHits,
          trinityRoomSealed: game.trinityRoomSealed,
          finalDoorOpen: game.finalDoorOpen
        });
      }
      ...
      function onKD(e) {
        ...
        if (e.code === "Space") {
          ...
          if (
            ...
            game.slowMo = true;
            game.slowMoTimer = 7;
            sfx.startSlowMoMusic();
          }
        }
      }
      function onKU(e) {
        game.kbd[e.code] = false;
      }
      var lastMouseX = -1,
        lastMouseY = -1;
      function onMD(e) {
      ...
      function trinityGunLocked() {
        ...
        if (
          game.trinityAgentsCleared &&
          game.trinityRoot &&
          game.trinityRoot.visible &&
          game.trinityPhase >= 0 &&
          game.trinityPhase < 3
        )
          return true;
      ...
      function playerShoot() {
        ...
        game.playerBullets.push({
          ...
          life: isMg ? 2.2 : 3,
          speed: bspd,
          prevX: bMesh.position.x,
          prevZ: bMesh.position.z
        });
      }
      ...
      function playerPunch() {
        ...
        var ppx = cam2.position.x,
          ppz = cam2.position.z;
        var s = game.agentSmith;
        var pdx = s.x - ppx,
          pdz = s.z - ppz;
        var pdist = Math.hypot(pdx, pdz);
        var pfwdx = -Math.sin(game.yaw),
          pfwdz = -Math.cos(game.yaw);
      ...
      function onPLC() {
        ...
        lastMouseX = -1;
        lastMouseY = -1;
        game.paused = isPausedAt({
          isMobile: game.isMobile,
          pointerLocked: !!document.pointerLockElement,
          won: game.won,
          caught: game.caught
        });
        sfx.setAudioPaused(game.paused);
      ...
      function gameLoop() {
        ...
        try {
          ...
          if (game.won || game.caught) {
            renderer.render(scene, camera);
            return;
          }
          if (game.paused) {
            // Freeze the simulation under the PAUSED overlay: swallow the
            // elapsed time so agents/bullets don't advance on resume.
            clock.getDelta();
            renderer.render(scene, camera);
            return;
          }
          ...
          // Ease the time scale toward 20% / 100% instead of snapping
          game.timeScale = approach(game.timeScale, game.slowMo ? 0.2 : 1, 12, realDt);
          if (!game.slowMo && game.timeScale > 0.995) game.timeScale = 1;
          dt = realDt * game.timeScale;

          // Dynamic resolution — step render scale down under sustained load
          if (updateResState(game.resState, realDt)) {
            renderer.setPixelRatio(effectivePixelRatio());
          }

          // Hit-feedback timers run on real time, not slow-mo time
          if (game.hitSfxCd > 0) game.hitSfxCd -= realDt;
          if (game.dmgFlash > 0) {
            game.dmgFlash = Math.max(0, game.dmgFlash - realDt);
            if (dmgVignetteRef.current)
              dmgVignetteRef.current.style.opacity = String(Math.min(1, game.dmgFlash * 2));
          }
          if (game.hitMark > 0) {
            game.hitMark = Math.max(0, game.hitMark - realDt);
            if (crosshairRef.current) {
              var hmScale = 1 + 0.6 * (game.hitMark / 0.12);
              crosshairRef.current.style.transform =
                "translate(-50%,-50%) scale(" + hmScale.toFixed(3) + ")";
            }
          }
          ...
          // Movement — always active during gameplay
          {
            ...
            if (!game.introDone && game.introPhase === 0) {
              fwd = 0;
              str = 0;
            }
            var trinitySeqMoveLock =
              game.trinityAgentsCleared &&
              game.trinityRoot &&
              game.trinityPhase >= 0 &&
              game.trinityPhase < 4 &&
            ...
            if (trinitySeqMoveLock || smithExitStepLock || game.playerFrozen) {
              fwd = 0;
              str = 0;
            }
            ...
            var sy = Math.sin(game.yaw),
              cy = Math.cos(game.yaw);
            ...
            // Sprint FOV kick — ease a little wider while sprinting and moving
            var fovTarget = settingsRef.current.fov + (isSprinting && il > 0.01 ? 7 : 0);
            if (Math.abs(camera.fov - fovTarget) > 0.02) {
              camera.fov = approach(camera.fov, fovTarget, 10, realDt);
              camera.updateProjectionMatrix();
            }
          }
          ...
          var allKeys =
            game.collected.has("green") && game.collected.has("red") && game.collected.has("blue");
          if (allKeys && game.glassCase && game.glassCase.visible) {
            ...
            for (var fhi = furnHits.length - 1; fhi >= 0; fhi--) {
              if (furnHits[fhi].x === 7 && furnHits[fhi].z === 10.5) {
                furnHits.splice(fhi, 1);
                break;
              }
            }
          ...
          if (!game.hasGun && game.gunMesh && !game.glassCase) {
            ...
            if (gpDist < 1.5) {
              ...
              if (game.doorMeshes["blue"]) {
                for (var dmi = 0; dmi < game.doorMeshes["blue"].length; dmi++)
                  scene.remove(game.doorMeshes["blue"][dmi]);
              }
          ...
          if (game.exitHallCaseOpened && game.exitHallMGMesh && !game.hasMachineGun) {
            ...
            if (mgPickD < 1.65) {
              ...
              if (!game.hallDoorOpen) {
                ...
                sfx.doorOpen();
                if (game.hallDoorMesh) {
                  scene.remove(game.hallDoorMesh);
                  game.hallDoorMesh = null;
                }
              }
          ...
          if (!game.introDone && game.morpheusRoot) {
            ...
            if (game.introPhase === 0) {
              ...
              var mtx = 1.5,
                mtz = 11.15;
              var mdx = mtx - mroot.position.x,
                mdz = mtz - mroot.position.z;
              ...
              if (game.introTimer >= 0.18 && game.boothHandsetParts) {
                for (var bhHi = 0; bhHi < game.boothHandsetParts.length; bhHi++)
                  game.boothHandsetParts[bhHi].visible = false;
              }
          ...
          for (var pbi = game.playerBullets.length - 1; pbi >= 0; pbi--) {
            ...
            var pbHit = false;
            var pbx2 = pb.mesh.position.x,
              pbz2 = pb.mesh.position.z,
              pby2 = pb.mesh.position.y;
            pb.prevX = pbx2;
            pb.prevZ = pbz2;
            if (segmentXZHitsBulletBlocks(ox, oz, pbx2, pbz2, pby2)) pbHit = true;
            if (
              trinityHostiles > 0 &&
              pbx2 > TRINITY_DOORWAY_X0 &&
              pbx2 < TRINITY_DOORWAY_X1 &&
              pbPrevZ <= TRINITY_ENTRY_Z &&
              pbz2 > TRINITY_ENTRY_Z &&
              pby2 > 0.12 &&
              pby2 < ROOM_H - 0.15
            ) {
              pbHit = true;
            ...
            for (var bhi = 0; bhi < game.agents.length; bhi++) {
              ...
              var bhDist = Math.sqrt(
                Math.pow(pb.mesh.position.x - bha.x, 2) + Math.pow(pb.mesh.position.z - bha.z, 2)
              );
            ...
            if (
              ...
              var trx = trHit.position.x,
                trz = trHit.position.z;
              ...
              if (triClose < 1.58 && pby2 > 0.12 && pby2 < 2.7) {
                ...
                if (game.trinityBubble) {
                  ...
                  if (game.trinityBubble.sprite.material)
                    game.trinityBubble.sprite.material.opacity = 1;
                }
                pbHit = true;
              }
            }
            ...
            if (!pbHit && game.cats) {
              for (var chi = 0; chi < game.cats.length; chi++) {
                ...
                var catDist = Math.sqrt(
                  Math.pow(pb.mesh.position.x - catH.x, 2) +
                    Math.pow(pb.mesh.position.z - catH.z, 2)
                );
            ...
            if (!pbHit && game.rabbit) {
              var rabDist = Math.sqrt(
                Math.pow(pb.mesh.position.x - game.rabbit.x, 2) +
                  Math.pow(pb.mesh.position.z - game.rabbit.z, 2)
              );
          ...
          for (var dai = game.agents.length - 1; dai >= 0; dai--) {
            ...
            if (da.deathTimer > 2) {
              scene.remove(da.group);
              game.agents.splice(dai, 1);
            }
          }
          ...
          if (game.trinityBetrayalAnim != null && game.trinityRoot) {
            ...
            if (!game.trinityBetrayalMats) {
              ...
              triFadeRoot.traverse(function (obj) {
                if (obj.isMesh && obj.material) {
                  var mm = obj.material;
                  if (mm.userData.trinityFadeBase == null)
                    mm.userData.trinityFadeBase = mm.opacity != null ? mm.opacity : 1;
                  mm.transparent = true;
            ...
            if (tbBase) {
              ...
              if (triBet && tbt < 7.55 && tbt - (triBet.lastPartT || 0) >= 0.48) {
                ...
                spawnHitParticles(
                  ...
                  tcy,
                  tcz,
                  Math.random() > 0.45 ? 0xff4488 : 0xff2233,
            ...
            if (tbt >= 7.55) {
              ...
              if (game.finalExitFrameMeshes) {
                for (var femb = 0; femb < game.finalExitFrameMeshes.length; femb++)
                  game.finalExitFrameMeshes[femb].visible = true;
              }
          ...
          if (game.agentSmith && game.smithPhase === 0) {
            ...
            sm1.leftLegPivot.rotation.x = THREE.MathUtils.lerp(
              sm1.leftLegPivot.rotation.x,
              0,
              Math.min(1, dt * 10)
            );
            sm1.rightLegPivot.rotation.x = THREE.MathUtils.lerp(
              sm1.rightLegPivot.rotation.x,
              0,
              Math.min(1, dt * 10)
            );
            ...
            if (game.smithPunchAnim > 0) sm3.rightArmPivot.rotation.x = -1.32;
              ...
              sm3.rightArmPivot.rotation.x = THREE.MathUtils.lerp(
                sm3.rightArmPivot.rotation.x,
                0,
                Math.min(1, dt * 8)
              );
            sm3.syncGroup();
            if (game.smithHitReactTimer > 0) {
              ...
              sm3.group.rotation.z = THREE.MathUtils.lerp(
                sm3.group.rotation.z,
                0,
                Math.min(1, dt * 10)
              );
            }
            ...
            if (game.smithDefeatSubPhase === 0) {
              ...
              if (fmul2 <= 0.04 || game.smithDefeatTimer > 2.75) {
                ...
                if (game.finalExitLight) game.finalExitLight.intensity = 1.15;
                if (game.finalExitFrameMeshes) {
                  for (var fem = 0; fem < game.finalExitFrameMeshes.length; fem++)
                    game.finalExitFrameMeshes[fem].visible = true;
                }
            ...
            game.punchVisualGroup.visible = false;
            game.punchVisualGroup.position.z =
              game.punchArmBaseZ != null ? game.punchArmBaseZ : -0.14;
            game.punchVisualGroup.rotation.x = 0;
            game.punchVisualGroup.rotation.y = 0;
          }
          ...
          if (smithHpBarRef.current && camera && game.agentSmith) {
            ...
            if (hpS) {
              ...
              if (smithHpFillRef.current)
                smithHpFillRef.current.style.width = (game.brawlSmithHp / 5) * 100 + "%";
            }
          }
          ...
          if (
            ...
            !game.caught &&
            game.secretMorpheusPhase === 0 &&
            pz > TRINITY_BETRAYAL_MORPHEUS_Z_FAR &&
            pz < TRINITY_BETRAYAL_MORPHEUS_Z_NEAR &&
            px >= 5.15 &&
            px <= 8.85
          ) {
            ...
            if (game.morpheusRoot) {
              ...
              if (game.reinstateMorpheusForBetrayalAmbush)
                game.reinstateMorpheusForBetrayalAmbush();
            }
            if (document.pointerLockElement) document.exitPointerLock();
            if (pauseRef.current) pauseRef.current.style.display = "none";
          }
          ...
          if (game.secretMorpheusPhase > 0 && game.morpheusRoot) {
            ...
            if (game.secretMorpheusPhase === 1) {
              ...
              if (mvd <= morStop + 0.04) {
                ...
                mrr.position.x = Math.max(
                  TRINITY_X0 + 0.42,
                  Math.min(TRINITY_X1 - 0.42, mrr.position.x)
                );
                mrr.position.z = Math.max(
                  TRINITY_Z0 + 0.45,
                  Math.min(mrr.position.z, MORPHEUS_BETRAYAL_WALK_MAX_Z)
                );
              }
          ...
          for (var a = 0; a < KEY_DEFS.length; a++) {
            ...
            if (Math.sqrt(Math.pow(px - kk.x, 2) + Math.pow(pz - kk.z, 2)) < 1.8) {
              ...
              if (kmm) {
                kmm.mesh.visible = false;
                kmm.ring.visible = false;
              }
              if (kk.id === "blue") {
                ...
                if (dms2) {
                  for (var q2 = 0; q2 < dms2.length; q2++) dms2[q2].visible = false;
                }
              }
              updateGame({
                keys: {
                  green: game.collected.has("green"),
                  red: game.collected.has("red"),
                  blue: game.collected.has("blue")
                },
                doors: {
                  green: game.opened.has("green"),
                  red: game.opened.has("red"),
                  blue: game.opened.has("blue")
                }
              });
            }
          }
          ...
          for (var ag = 0; ag < game.agents.length; ag++) {
            ...
            if (playerInRoom) {
              ...
              var ptDist = Math.sqrt(
                Math.pow(agent.targetX - agent.x, 2) + Math.pow(agent.targetZ - agent.z, 2)
              );
            ...
            if (tDist > 0.1 && moveSpd > 0) {
              ...
              var distToTarget = Math.sqrt(
                Math.pow(moveToX - agent.x, 2) + Math.pow(moveToZ - agent.z, 2)
              );
              var movedDist = Math.sqrt(
                Math.pow(agent.x - prevX, 2) + Math.pow(agent.z - prevZ, 2)
              );
              ...
              if (agent.stuckTime > 0.5 && (!agent.wpQueue || agent.wpQueue.length === 0)) {
                ...
                for (var fb = 0; fb < furnHits.length; fb++) {
                  ...
                  if (fDist < 6) {
                    var toTargetDist = Math.sqrt(
                      Math.pow(ff.x - moveToX, 2) + Math.pow(ff.z - moveToZ, 2)
                    );
                ...
                if (nearestBlocker) {
                  ...
                  for (var ci4 = 0; ci4 < 4; ci4++) {
                    var d2 = Math.sqrt(
                      Math.pow(c[ci4].x - agent.x, 2) + Math.pow(c[ci4].z - agent.z, 2)
                    );
                    if (d2 < nearD) {
                      nearD = d2;
                      nearIdx = ci4;
                  ...
                  for (var ci5 = 0; ci5 < 4; ci5++) {
                    var d3 = Math.sqrt(
                      Math.pow(c[ci5].x - moveToX, 2) + Math.pow(c[ci5].z - moveToZ, 2)
                    );
                    if (d3 < farD) {
                      farD = d3;
                      farIdx = ci5;
            ...
            if (faceDist > 0.1 && !agent.hasGun)
              agent.group.rotation.y = Math.atan2(faceDx, faceDz);
          ...
          for (var bi = game.bullets.length - 1; bi >= 0; bi--) {
            ...
            var agNx = bul.mesh.position.x,
              agNz = bul.mesh.position.z;
            ...
            // Player collision
            var bpDist = Math.sqrt(
              Math.pow(bul.mesh.position.x - px, 2) + Math.pow(bul.mesh.position.z - pz, 2)
            );
            if (bpDist < 0.5) {
          ...
          for (var catI = 0; catI < game.cats.length; catI++) {
            ...
            if (ctDist > 0.1) {
              ...
              if (!catBlocked(cnx, cnz)) {
                cat.x = cnx;
                cat.z = cnz;
              } else if (!catBlocked(cnx, cat.z)) {
                cat.x = cnx;
              } else if (!catBlocked(cat.x, cnz)) {
                cat.z = cnz;
              } else {
                cat.targetX = cat.minX + Math.random() * (cat.maxX - cat.minX);
                cat.targetZ = cat.minZ + Math.random() * (cat.maxZ - cat.minZ);
            ...
            if (cat.sayTimer <= 0) {
              if (cat.bubble.sprite.visible) {
                ...
                updateBubbleText(
                  cat.bubble,
                  CAT_QUOTES[Math.floor(Math.random() * CAT_QUOTES.length)]
                );
          ...
          if (slowMoTextRef.current) {
            if (game.slowMo) {
              ...
              slowMoTextRef.current.textContent =
                "MATRIX TIME ACTIVE — " + Math.ceil(game.slowMoTimer) + "s";
            } else {
          ...
          if (slowMoBtnRef.current) {
            if (game.slowMo) {
              ...
              slowMoBtnRef.current.textContent =
                "COOLDOWN: " + Math.ceil(game.slowMoCooldown) + "s";
          ...
          if (game.finalDoorOpen && game.finalExitGlow) {
            ...
            if (game.finalExitLight)
              game.finalExitLight.intensity = 0.95 + 0.35 * Math.sin(animT * 2.2);
            if (game.finalExitFrameMeshes) {
              for (var fem2 = 0; fem2 < game.finalExitFrameMeshes.length; fem2++) {
                fmesh = game.finalExitFrameMeshes[fem2];
                fmesh.visible = true;
                if (fmesh.material) {
          ...
          try {
            showRoom("RUNTIME ERROR — CHECK CONSOLE");
          } catch {
            /* ignore */
          }
    ...
    [entered, game, spawnHitParticles, updateGame]
  );
  ...
  var objectives = [
    ...
    {
      text: "Secret 1/3 — Shoot a cat or the white rabbit (dark ending)",
      done: false,
      secretHint: true
    },
    {
      text: "Secret 2/3 — Within 10s after Morpheus jacks out, step into the phone booth",
      done: false,
      secretHint: true
    },
    {
      text: "Secret 3/3 — While Trinity runs for the exit, shoot her, then approach the final exit",
      done: false,
      secretHint: true
    }
  ];
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: "hidden",
        touchAction: "none",
        cursor: entered && !gs.won && !gs.caught ? "none" : "default"
      }}
    >
      ...
      {!entered && (
        ...
        >
          ...
          >
            Morpheus opens the mission. Find keys, reach Trinity in the corridor, free her, then
            take the final exit out of the Matrix.
          </p>
          <div
            style={{
              fontFamily: font,
          ...
          >
            {isMob ? (
              <React.Fragment>
                ...
                <div>DRAG ANYWHERE — Look</div>
                <div>SPACE — Matrix Time</div>
              </React.Fragment>
            ) : (
              <React.Fragment>
                <div>WASD — Move</div>
      ...
      {gs.won && (
        ...
        >
          ...
          >
            {String(Math.floor(gs.time / 60)).padStart(2, "0")}:
            {String(gs.time % 60).padStart(2, "0")}
          </p>
          <p
            style={{
              fontFamily: font,
      ...
      {gs.caught && (
        <div
          style={{
            ...
            alignItems: "center",
            justifyContent: "center",
            background: gs.caughtReason ? "rgba(0,5,10,0.85)" : "rgba(10,0,0,0.8)",
            zIndex: 100,
            overflow: "hidden"
          }}
        >
          ...
          {gs.caughtReason === "animal_secret_cat" ? (
            <React.Fragment>
              ...
              >
                The Matrix has reported you to PETA.
              </p>
              <p
                style={{
                  fontFamily: font,
              ...
              >
                That cat had 8 lives left, you monster.
              </p>
            </React.Fragment>
            ...
            <React.Fragment>
              ...
              >
                Morpheus is VERY disappointed in you.
              </p>
              <p
                style={{
                  fontFamily: font,
              ...
              >
                You were supposed to FOLLOW it, not SHOOT it.
              </p>
            </React.Fragment>
            ...
            <React.Fragment>
              ...
              >
                You stepped into the booth within 10 seconds and left the simulation early.
              </p>
              <p
                style={{
                  fontFamily: font,
              ...
              >
                Trinity is still in the Matrix. Hope that call was worth it.
              </p>
            </React.Fragment>
            ...
            <React.Fragment>
              ...
              >
                Even the Chosen One answers for desecrating Trinity.
              </p>
              <p
                style={{
                  fontFamily: font,
              ...
              >
                Machine-gunned by the mentor. The Architect sends a sympathy card.
              </p>
            </React.Fragment>
            ...
            <React.Fragment>
              ...
              >
                Agent Smith connected five times. You agreed to the rules.
              </p>
              <p
                style={{
                  fontFamily: font,
              ...
              >
                The Matrix sends its regards — politely, of course.
              </p>
            </React.Fragment>
          ) : (
            <React.Fragment>
          ...
          <div
            ...
            style={{
              ...
              color: "#000",
              background:
                gs.caughtReason === "brawl" ? "#ff6666" : gs.caughtReason ? "#ff9900" : "#ff3333",
              padding: "12px 32px",
              fontSize: "clamp(0.8rem,3vw,1rem)",
              cursor: "pointer",
              letterSpacing: "0.15em",
              boxShadow:
                "0 0 20px " +
                (gs.caughtReason === "brawl" ? "#ff4444" : gs.caughtReason ? "#ff6600" : "#ff0000"),
              position: "relative",
              zIndex: 1
            }}
          >
      ...
      {entered && (
        <div
          ...
          style={{
            ...
            inset: 0,
            background:
              "radial-gradient(ellipse at center, rgba(255,0,0,0) 55%, rgba(255,30,30,0.55) 100%)",
            opacity: 0,
            zIndex: 64,
            pointerEvents: "none"
          }}
      ...
      {entered && !gs.won && !gs.caught && !gs.brawlActive && (
        <div
          ...
          style={{
            ...
            width: gs.hasGun ? 24 : 20,
            height: gs.hasGun ? 24 : 20,
            zIndex: 65,
            pointerEvents: "none"
          }}
        >
          <div
            style={{
              ...
              background: gs.hasGun ? "rgba(255,50,50,0.8)" : "rgba(0,255,65,0.5)"
            }}
          />
          <div
            style={{
          ...
          {gs.hasGun && (
            <div
              style={{
                ...
                left: "50%",
                transform: "translate(-50%,-50%)",
                width: gs.hasMachineGun ? 5 : 4,
                height: gs.hasMachineGun ? 5 : 4,
                borderRadius: "50%",
                background: gs.hasMachineGun ? "rgba(255,170,50,0.75)" : "rgba(255,50,50,0.6)"
              }}
            />
          )}
        </div>
      ...
      {entered && !gs.won && !gs.caught && (
        ...
        >
          ...
          >
            <div
              ...
              style={{
                height: "100%",
                width: "100%",
                background: "linear-gradient(90deg,#006622,#00ee66)",
          ...
          >
            Heals after ~2.5s out of fire — three clean hits can drop you
          </div>
        </div>
      )}
        ...
        >
          ...
          <div
            style={{
              ...
              border: "1px solid rgba(160,180,200,0.55)",
              borderRadius: 2,
              overflow: "hidden"
            }}
          >
            <div
              ...
              style={{
                height: "100%",
                width: "100%",
                background: "linear-gradient(90deg,#334455,#99aabb)",
      ...
      {entered && !gs.won && !gs.caught && isMob && gs.brawlActive && (
        <div
          onClick={function (ev) {
            ev.stopPropagation();
            if (game.playerPunchFn) game.playerPunchFn();
          }}
          onTouchEnd={function (ev) {
            ev.preventDefault();
            ev.stopPropagation();
      ...
      {entered && !gs.won && !gs.caught && (
        <div
          style={{ position: "absolute", top: 12, right: 12, zIndex: 80, pointerEvents: "auto" }}
        >
          <div
            onClick={function () {
              setShowObj(!showObj);
            ...
            >
              ...
              {isMob ? (
                <React.Fragment>
                  ...
                  >
                    PUNCH button — Strike in Smith brawl
                  </div>
                  <div
                    style={{
                      fontFamily: font,
                ...
                <React.Fragment>
                  ...
                  >
                    Click / PUNCH — strike during Smith brawl
                  </div>
                  <div
                    style={{
                      fontFamily: font,
                  ...
                  >
                    Hold click — machine gun auto-fire
                  </div>
                  <div
                    style={{
                      fontFamily: font,
          ...
          {showOpts && (
            ...
            >
              ...
              <input
                ...
                onChange={function (e) {
                  settingsRef.current.sensitivity = parseFloat(e.target.value);
                }}
                onPointerDown={function (e) {
                  e.stopPropagation();
                }}
              ...
              <input
                ...
                defaultValue="1"
                onChange={function (e) {
                  settingsRef.current.brightness = parseFloat(e.target.value);
                }}
                onPointerDown={function (e) {
                  e.stopPropagation();
                }}
              ...
              <input
                ...
                defaultValue="1"
                onChange={function (e) {
                  settingsRef.current.enemySpeed = parseFloat(e.target.value);
                }}
                onPointerDown={function (e) {
                  e.stopPropagation();
                }}
              ...
              <input
                ...
                defaultValue="75"
                onChange={function (e) {
                  settingsRef.current.fov = parseFloat(e.target.value);
                }}
                onPointerDown={function (e) {
                  e.stopPropagation();
                }}
              ...
              >
                <input
                  ...
                  onChange={function (e) {
                    settingsRef.current.invertY = e.target.checked;
                  }}
                  onPointerDown={function (e) {
                    e.stopPropagation();
                  }}
              ...
              >
                <input
                  ...
                  defaultChecked={true}
                  onChange={function (e) {
                    settingsRef.current.soundOn = e.target.checked;
                    sfx.setAudioEnabled(e.target.checked);
                  }}
                  onPointerDown={function (e) {
                    e.stopPropagation();
                  }}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  height: 14,
                  borderRadius: "50%",
                  border: "2px solid " + COLOR_CSS[kc],
                  background: gs.keys[kc] ? COLOR_CSS[kc] : "transparent",
                  boxShadow: gs.keys[kc] ? "0 0 8px " + COLOR_CSS[kc] : "none",
                  transition: "all 0.3s"
                }}
              />
            );
          ...
          <div
            ...
            onClick={function () {
              ...
              if (sprintBtnRef.current) {
                sprintBtnRef.current.style.background = game.sprinting
                  ? "rgba(0,180,0,0.4)"
                  : "rgba(0,40,0,0.5)";
                sprintBtnRef.current.style.borderColor = game.sprinting ? "#00ff41" : "#006600";
                sprintBtnRef.current.textContent = game.sprinting ? "SPRINT: ON" : "SPRINT";
              }
            }}
            onTouchEnd={function (ev) {
              ev.preventDefault();
          ...
          <div
            onPointerDown={function (ev) {
              ...
              if (
                ...
                game.caught ||
                game.playerFrozen ||
                (game.trinityGunLockedFn && game.trinityGunLockedFn())
              )
                return;
              if (game.hasMachineGun) game.fireHeld = true;
            ...
            style={{
              ...
              color: "#ff4444",
              fontSize: "clamp(0.55rem,2vw,0.75rem)",
              background: gs.hasGun ? "rgba(80,0,0,0.5)" : "rgba(30,30,30,0.3)",
              border: "1px solid " + (gs.hasGun ? "#cc0000" : "#333333"),
              padding: "8px 14px",
              cursor: "pointer",
              letterSpacing: "0.1em",
              touchAction: "none",
      ...
      {entered && !isMob && !gs.won && !gs.caught && (
        ...
        >
          ...
          >
            Click to resume — Escape to pause
          </div>
        </div>
      )}
    </div>
